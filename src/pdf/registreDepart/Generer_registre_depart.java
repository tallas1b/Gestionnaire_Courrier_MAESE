...
public class Generer_registre_depart extends Service<Boolean>{
		...
		public Generer_registre_depart(List<Beans_Message_Depart> list , File file) {
		...
		public boolean create_file(File f) {
			...
			try {
				...
				//add model concerne
				Paragraph para_model = creat_paragraphe_model_concerne("Message départ");
		...
		private Table creat_table(List<Beans_Message_Depart> list) {
			try {
				...
				float [] pointColumnWidths = {25F, 40F,45F,50F, 70F,50F,40F,380F};
				...
				Cell cell_nom_fichier = new Cell().add(new Paragraph("Fichier").setFont(font).setFontSize(8));
				cell_nom_fichier.setTextAlignment(TextAlignment.CENTER);
				table.addHeaderCell(cell_nom_fichier);
		...
		private void new_populate_table(Table table ,List<Beans_Message_Depart> list) {
			...
			for(int i=0; i< list.size(); i++) {
				//num depart
				Cell cell = new Cell().add(new Paragraph(list.get(i).getNumero_depart()+"").setFontSize(8));
			...
			//	cell.setBorder(Border.NO_BORDER);
				...
				//cell_destinataire.setBorder(Border.NO_BORDER);
				...
				//Nom fichier
				Cell nom_fichier = new Cell().add(new Paragraph(list.get(i).getNom_fichier()+"").setFontSize(8));
				nom_fichier.setTextAlignment(TextAlignment.CENTER);
				table.addCell(nom_fichier);
				...
				//cell_mention.setBorder(Border.NO_BORDER);
				...
				//cell_objet.setBorder(Border.NO_BORDER);
